...
logic_e isTrackDone;					// 全局 跟踪完成标志位
...
static path_t test_path[TEST_PATH_NUMBER] = {
	...
	{0.00f, 0.00f, 0.00f, 2.00f, NONE, NONE, NONE, NONE},
	{0.00f, 2.00f, 1.00f, 3.00f, 1.00f, 2.00f, 1.00f, CW},
	{1.00f, 3.00f, 3.00f, 3.00f, NONE, NONE, NONE, NONE},
	{3.00f, 3.00f, 4.00f, 2.00f, 3.00f, 2.00f, 1.00f, CW},
	{4.00f, 2.00f, 4.00f, 0.00f, NONE, NONE, NONE, NONE},
	{4.00f, 0.00f, 3.00f, -1.00f, 3.00f, 0.00f, 1.00f, CW},
	{3.00f, -1.00f, 1.00f, -1.00f, NONE, NONE, NONE, NONE},
	{1.00f, -1.00f, 0.00f, 0.00f, 1.00f, 0.00f, 1.00f, CW},
};
...
void navigation_task(void)
{
	...
	case NAV_STATE_INIT: //********** 初始化状态 **********
		for (path_index = 0; path_index < TEST_PATH_NUMBER; path_index++)
		{
			...
			path_init(&test_path[path_index], &interpolation_data[path_index]);
#ifdef DEBUG
	...
	case NAV_STATE_RUN: //********** 运行状态 **********
		//判断一段插补是否结束 结束的话就 把 path_index 加一进入下一段插补
		...
		/* 更新世界坐标 */
		pps_feedback.pos_y = global_position.pos_y;
		pps_feedback.pos_x = global_position.pos_x;
		pps_feedback.theta = global_position.theta;
		/* 轨迹跟踪控制器 */
		track_controller(&test_path[path_index], &interpolation_data[path_index], &pps_feedback, &vx_set, &vy_set, &wz_set);
		/* x-m/s  y-m/s  wz-rad/s */
...
/**
  ...
  * @retval         none
  */
void path_init(path_t *path, interpolation_t *inp_data)
{
...
void path_interpolation(path_t *path, interpolation_t *inp_data)
{
	...
	if (inp_data->residue_theta > Round_Error_T)
	{
		dtheta = 0;
		pre_w_z = inp_data->w_z;
		inp_data->brake_theta = fabs(pow(inp_data->w_z, 2) - pow(path->w_end, 2)) / (W_ACC * 2);
		//剩余角度大于理论刹车角度 将速度逐步的调整为给定的目标(最大)角速度 可以加速、减速、匀速
		if (inp_data->residue_theta - inp_data->brake_theta > Round_Error)
		{
			if (inp_data->w_z < path->w_target) //加速
			{
				inp_data->w_z += W_ACC * TRAIL_REVISE_TIME;
				high_constrain(inp_data->w_z, path->w_target);
				dtheta = fabs(pow(inp_data->w_z, 2) - pow(pre_w_z, 2)) / (2 * W_ACC);
			}
			else if (inp_data->w_z > path->w_target) //减速
			{
				inp_data->w_z -= W_ACC * TRAIL_REVISE_TIME;
				low_constrain(inp_data->w_z, path->w_target);
				dtheta = fabs(pow(inp_data->w_z, 2) - pow(pre_w_z, 2)) / (2 * W_ACC);
			}
			else //匀速(达到目标速度)
			{
				inp_data->w_z = path->w_target;
				dtheta = fabs(inp_data->w_z) * TRAIL_REVISE_TIME;
			}
		}
		...
		{
			//更改加速度，使小车可以在终点速度恰好为要求的速度
			w_acc_adjusted = fabs(pow(inp_data->w_z, 2) - pow(path->w_end, 2)) / (2 * inp_data->residue_theta);
			if (inp_data->w_z < path->w_end) //加速
			{
				inp_data->w_z += w_acc_adjusted * TRAIL_REVISE_TIME;
				high_constrain(inp_data->w_z, path->w_end);
				dtheta = fabs(pow(inp_data->w_z, 2) - pow(pre_w_z, 2)) / (2 * w_acc_adjusted);
			}
			else if (inp_data->w_z > path->w_end) //减速
			{
				inp_data->w_z -= w_acc_adjusted * TRAIL_REVISE_TIME;
				low_constrain(inp_data->w_z, path->w_end);
				dtheta = fabs(pow(inp_data->w_z, 2) - pow(pre_w_z, 2)) / (2 * w_acc_adjusted);
			}
			else //匀速(达到目标速度)
			{
				inp_data->w_z = path->w_end;
				dtheta = inp_data->w_z * TRAIL_REVISE_TIME;
			}
		}

		//逆时针为正 顺时针为负
		//(path->rotate_direction == CW) ? (dtheta = -dtheta) : (dtheta = dtheta);

		//更新已经插补过的角度(rad) 已经插补的角度 总角度 当前角度 都是有正有负 有方向的量
		inp_data->past_theta += dtheta;
		inp_data->residue_theta = inp_data->total_theta - inp_data->past_theta; //剩余角度用的是绝对角度 正值

		//判断结束条件
		// if ((path->rotate_direction == CW && inp_data->past_theta <= inp_data->total_theta) ||
		// 	(path->rotate_direction == CCW && inp_data->past_theta >= inp_data->total_theta) ||
		// 	(inp_data->residue_theta <= Round_Error))
		if (inp_data->residue_theta <= Round_Error_T)
		{
			inp_data->past_theta = inp_data->total_theta;
			inp_data->residue_theta = 0;
			inp_data->w_z = path->w_end;
			inp_data->theta_interpolation_is_done = TRUE; //自转角度插补完成 标志
		}
		else //插补还未结束
		{
			if (path->rotate_direction == CCW)
				inp_data->theta = inp_data->past_theta + path->start_theta; // 计算当前点 应该转到的角度
			else
			{
				inp_data->theta = -inp_data->past_theta + path->start_theta; // 计算当前点 应该转到的角度
			}
